...
# --- Core Logic Execution Block ---
extracted_passwords = []
ssids = []
process_successful = False # Flag to indicate if we reached the final file writing stage

try:
    # --- 1. Get WiFi Profiles ---
    # Get list of WiFi profiles/connections
    ...
    # If the initial command fails, we exit the try block, moving to finally
    
except KeyboardInterrupt:
    print("\n[STOPPED] User interrupted the extraction process.")
    # Exit the try block, moving to finally
    
finally:
    # --- 3. Final Processing and Cleanup (Runs on success or interrupt) ---
